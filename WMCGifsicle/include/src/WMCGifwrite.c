...
#define NODES_SIZE		WMC_GIF_MAX_CODE
#define LINKS_SIZE		WMC_GIF_MAX_CODE


#define TABLE_TYPE		0
#define LINKS_TYPE		1
#define MAX_LINKS_TYPE		5
typedef struct WMC_Node {
  WMC_Code code;
  uint8_t type;
  uint8_t suffix;
  struct WMC_Node *sibling;
  union {
    struct WMC_Node *s;
    struct WMC_Node **m;
  } child;
...
struct WMC_Writer {
  ...
  uint32_t pos;
  uint32_t cap;
  WMC_CompressInfo gcinfo;
  int global_size;
  int local_size;
  int errors;
  int cleared;
  WMC_CodeTable code_table;
  void (*byte_putter)(uint8_t, struct WMC_Writer *);
  void (*block_putter)(const uint8_t *, size_t, struct WMC_Writer *);
};
...
static inline void
gifputunsigned(uint16_t uns, WMC_Writer *grr)
{
...
static void
file_byte_putter(uint8_t b, WMC_Writer *grr)
{
  fputc(b, grr->f);
}

static void
file_block_putter(const uint8_t *block, size_t size, WMC_Writer *grr)
{
...
static void
memory_byte_putter(uint8_t b, WMC_Writer *grr)
{
  if (grr->pos >= grr->cap) {
    grr->cap = (grr->cap ? grr->cap * 2 : 1024);
    WMC_ReArray(grr->v, uint8_t, grr->cap);
  }
  if (grr->v) {
    grr->v[grr->pos] = b;
    grr->pos++;
...
static void
memory_block_putter(const uint8_t *data, size_t len, WMC_Writer *grr)
{
...
static int
gif_writer_init(WMC_Writer* grr, FILE* f, const WMC_CompressInfo* gcinfo)
{
  grr->f = f;
  grr->v = NULL;
  grr->pos = grr->cap = 0;
  if (gcinfo)
    ...
    WMC_InitCompressInfo(&grr->gcinfo);
  grr->errors = 0;
  grr->cleared = 0;
  grr->code_table.nodes = WMC_NewArray(WMC_Node, NODES_SIZE);
  grr->code_table.links = WMC_NewArray(WMC_Node*, LINKS_SIZE);
  if (f) {
...
static void
gif_writer_cleanup(WMC_Writer* grr)
{
  WMC_DeleteArray(grr->v);
  WMC_DeleteArray(grr->code_table.nodes);
  WMC_DeleteArray(grr->code_table.links);
}


static inline void
gfc_clear(WMC_CodeTable *gfc, WMC_Code clear_code)
{
  int c;
...
static inline WMC_Node *
gfc_lookup(WMC_CodeTable *gfc, WMC_Node *node, uint8_t suffix)
{
...
static inline unsigned int color_diff(WMC_Color a, WMC_Color b, int a_transaprent, int b_transparent, gfc_rgbdiff dither)
{
...
static inline gfc_rgbdiff diffused_difference(WMC_Color a, WMC_Color b, int a_transaprent, int b_transaprent, gfc_rgbdiff dither)
{
...
static inline uint8_t gif_pixel_at_pos(WMC_Image *gfi, unsigned pos);

static void
gfc_change_node_to_table(WMC_CodeTable *gfc, WMC_Node *work_node,
                         WMC_Node *next_node)
{
  /* change links node to table node */
  WMC_Code c;
  WMC_Node **table = &gfc->links[gfc->links_pos];
  WMC_Node *n;
...
static inline void
gfc_define(WMC_CodeTable *gfc, WMC_Node *work_node, uint8_t suffix,
           WMC_Code next_code)
{
  /* Add a new code to our dictionary. First reserve a node for the
     added code. It's LINKS_TYPE at first. */
  WMC_Node *next_node = &gfc->nodes[gfc->nodes_pos];
  gfc->nodes_pos++;
...
gif_imageline(WMC_Image *gfi, unsigned pos)
{
  unsigned y, x;
  if (gfi->width == 0)
    return NULL;
  ...
  if (y == (unsigned) gfi->height)
    ...
    return gfi->img[WMC_InterlaceLine(y, gfi->height)] + x;
}

static inline unsigned
gif_line_endpos(WMC_Image *gfi, unsigned pos)
{
...
struct selected_node {
  WMC_Node *node; /* which node has been chosen by gfc_lookup_lossy */
  unsigned long pos, /* where the node ends */
  diff; /* what is the overall quality loss for that node */
};

static inline void
gfc_lookup_lossy_try_node(WMC_CodeTable *gfc, const WMC_Colormap *gfcm, WMC_Image *gfi,
  unsigned pos, WMC_Node *node, uint8_t suffix, uint8_t next_suffix,
...
gfc_lookup_lossy(WMC_CodeTable *gfc, const WMC_Colormap *gfcm, WMC_Image *gfi,
  unsigned pos, WMC_Node *node, unsigned long base_diff, gfc_rgbdiff dither, const unsigned int max_diff)
{
...
gif_pixel_at_pos(WMC_Image *gfi, unsigned pos)
{
...
static int
write_compressed_data(WMC_Stream *gfs, WMC_Image *gfi,
		      int min_code_bits, WMC_Writer *grr)
{
  WMC_CodeTable* gfc = &grr->code_table;
...
#define RUN_INV_THRESH ((unsigned) (1 << RUN_EWMA_SCALE) / 3000)
  ...
  WMC_Node *work_node;
  WMC_Node *next_node;
  WMC_Code next_code = 0;
  WMC_Code output_code;
  uint8_t suffix;
...
#define CLEAR_CODE      ((WMC_Code) (1 << min_code_bits))
#define EOI_CODE        ((WMC_Code) (CLEAR_CODE + 1))
#define CUR_BUMP_CODE   (1 << cur_code_bits)
  ...
  while (1) {
    ...
    if (bufpos + 32 >= bufcap) {
      ...
      uint8_t *nbuf = WMC_NewArray(uint8_t, ncap >> 3);
      if (!nbuf)
        goto error;
    ...
    if (output_code == CLEAR_CODE) {
      ...
      if (next_code > CUR_BUMP_CODE && cur_code_bits < WMC_GIF_MAX_CODE_BITS)
    ...
    if (grr->gcinfo.loss) {
      ...
      work_node = t.node;
      run = (unsigned)t.pos - pos;
      pos = (unsigned)t.pos;

      if (pos < image_endpos) {
        /* Output the current code. */
        if (next_code < WMC_GIF_MAX_CODE) {
          gfc_define(gfc, work_node, gif_pixel_at_pos(gfi, pos), next_code);
          next_code++;
        } else
          next_code = WMC_GIF_MAX_CODE + 1; /* to match "> CUR_BUMP_CODE" above */

        /* Check whether to clear table. */
        if (next_code > 4094) {
          int do_clear = grr->gcinfo.flags & WMC_GIF_WRITE_EAGER_CLEAR;
...
static int
calculate_min_code_bits(WMC_Image *gfi, const WMC_Writer *grr)
{
  int colors_used = -1, min_code_bits, i;

  if (grr->gcinfo.flags & WMC_GIF_WRITE_CAREFUL_MIN_CODE_SIZE) {
...
static int get_color_table_size(const WMC_Stream *gfs, WMC_Image *gfi,
				WMC_Writer *grr);

static void
save_compression_result(WMC_Image *gfi, WMC_Writer *grr, int ok)
{
  if (!(grr->gcinfo.flags & WMC_GIF_WRITE_SHRINK)
...
int
WMC_FullCompressImage(WMC_Stream *gfs, WMC_Image *gfi,
		      const WMC_CompressInfo *gcinfo)
{
  ...
  if ((grr.gcinfo.flags & (WMC_GIF_WRITE_OPTIMIZE | WMC_GIF_WRITE_EAGER_CLEAR))
      == WMC_GIF_WRITE_OPTIMIZE
      && grr.cleared && ok) {
    grr.gcinfo.flags |= WMC_GIF_WRITE_EAGER_CLEAR | WMC_GIF_WRITE_SHRINK;
...
static int
get_color_table_size(const WMC_Stream *gfs, WMC_Image *gfi, WMC_Writer *grr)
{
  WMC_Colormap *gfcm = (gfi ? gfi->local : gfs->global);
  int ncol, totalcol, i;
...
static void
write_color_table(WMC_Colormap *gfcm, int totalcol, WMC_Writer *grr)
{
  WMC_Color *c = gfcm->col;
...
static int
write_image(WMC_Stream *gfs, WMC_Image *gfi, WMC_Writer *grr)
{
  ...
  /* calculate min_code_bits here (because calculation may involve
     recompression, if WMC_GIF_WRITE_CAREFUL_MIN_CODE_SIZE is true) */
  min_code_bits = calculate_min_code_bits(gfi, grr);
  ...
  if (gfi->compressed
      && (!(grr->gcinfo.flags & WMC_GIF_WRITE_CAREFUL_MIN_CODE_SIZE)
...
static void
write_logical_screen_descriptor(WMC_Stream *gfs, WMC_Writer *grr)
{
  uint8_t packed = 0x70;		/* high resolution colors */
  grr->global_size = get_color_table_size(gfs, 0, grr);

  WMC_CalculateScreenSize(gfs, 0);
...
static void
write_graphic_control_extension(WMC_Image *gfi, WMC_Writer *grr)
{
  uint8_t packed = 0;
  gifputbyte('!', grr);
  gifputbyte(0xF9, grr);
...
static void
blast_data(const uint8_t *data, int len, WMC_Writer *grr)
{
  while (len > 0) {
...
static void
write_name_extension(char *id, WMC_Writer *grr)
{
  gifputbyte('!', grr);
  gifputbyte(0xCE, grr);
  blast_data((uint8_t *)id, (int)strlen(id), grr);
}


static void
write_comment_extensions(WMC_Comment *gfcom, WMC_Writer *grr)
{
  int i;
...
static void
write_netscape_loop_extension(uint16_t value, WMC_Writer *grr)
{
...
static void
write_generic_extension(WMC_Extension *gfex, WMC_Writer *grr)
{
  uint32_t pos = 0;
...
static int
write_gif(WMC_Stream *gfs, WMC_Writer *grr)
{
  WMC_Extension* gfex;
  int ok = 0;
  int i;
  ...
  for (i = 0; i < gfs->nimages; i++)
    if (!WMC_IncrementalWriteImage(grr, gfs, gfs->images[i]))
      goto done;
...
int
WMC_FullWriteFile(WMC_Stream *gfs, const WMC_CompressInfo *gcinfo,
		  FILE *f)
{
  WMC_Writer grr;
...
WMC_Writer*
WMC_IncrementalWriteFileInit(WMC_Stream* gfs, const WMC_CompressInfo* gcinfo,
                             FILE *f)
{
    WMC_Writer* grr = WMC_New(WMC_Writer);
...
int
WMC_IncrementalWriteImage(WMC_Writer* grr, WMC_Stream* gfs, WMC_Image* gfi)
{
    WMC_Extension *gfex;
...
int
WMC_IncrementalWriteComplete(WMC_Writer* grr, WMC_Stream* gfs)
{
    WMC_Extension* gfex;
    for (gfex = gfs->end_extension_list; gfex; gfex = gfex->next)
        write_generic_extension(gfex, grr);
...
#undef WMC_CompressImage
#undef WMC_WriteFile

int
WMC_CompressImage(WMC_Stream *gfs, WMC_Image *gfi)
{
  return WMC_FullCompressImage(gfs, gfi, 0);
}

int
WMC_WriteFile(WMC_Stream *gfs, FILE *f)
{
  return WMC_FullWriteFile(gfs, 0, f);
}
