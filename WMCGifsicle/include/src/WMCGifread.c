...
typedef struct WMC_Reader {
  ...
  int is_record;
  int is_eoi;
  uint8_t (*byte_getter)(struct WMC_Reader *);
  uint32_t (*block_getter)(uint8_t*, uint32_t, struct WMC_Reader*);
  int (*eofer)(struct WMC_Reader *);
} WMC_Reader;

static WMC_ReadErrorHandler default_error_handler = 0;
...
static uint8_t
file_byte_getter(WMC_Reader *grr)
{
    int i = getc(grr->f);
    if (i != EOF) {
        ++grr->pos;
...
static uint32_t
file_block_getter(uint8_t *p, uint32_t s, WMC_Reader *grr)
{
...
static uint8_t
record_byte_getter(WMC_Reader *grr)
{
...
static uint32_t
record_block_getter(uint8_t *p, uint32_t s, WMC_Reader *grr)
{
...
static void
make_data_reader(WMC_Reader *grr, const uint8_t *data, uint32_t length)
{
  grr->v = data;
  grr->pos = 0;
  grr->length = length;
...
static void
gif_read_error(WMC_Context *gfc, int is_error, const char *text)
{
    WMC_ReadErrorHandler handler = gfc->handler ? gfc->handler : default_error_handler;
    if (is_error >= 0)
...
static uint8_t
one_code(WMC_Context *gfc, WMC_Code code)
{
  uint8_t *suffixes = gfc->suffix;
  WMC_Code *prefixes = gfc->prefix;
  uint8_t *ptr;
  int lastsuffix = 0;
...
static int
read_image_block(WMC_Reader *grr, uint8_t *buffer, int *bit_pos_store,
...
static void
read_image_data(WMC_Context *gfc, WMC_Reader *grr)
{
  /* we need a bit more than WMC_GIF_MAX_BLOCK in case a single code is split
     across blocks */
  uint8_t buffer[WMC_GIF_MAX_BLOCK + 5];
  int i;
  uint32_t accum;
...
#define CUR_CODE_MASK ((1 << bits_needed) - 1)
  ...
  if (min_code_size >= WMC_GIF_MAX_CODE_BITS) {
    gif_read_error(gfc, 1, "image corrupted, min_code_size too big");
    min_code_size = WMC_GIF_MAX_CODE_BITS - 1;
  ...
  while (1) {
    ...
    if (bits_needed >= 8)
      accum |= (buffer[i+2]) << 16;
    code = (WMC_Code)((accum >> (bit_position % 8)) & CUR_CODE_MASK);
	...
	if (bits_needed < WMC_GIF_MAX_CODE_BITS)
	  bits_needed++;
	else
...
static WMC_Colormap *
read_color_table(int size, WMC_Reader *grr)
{
  WMC_Colormap *gfcm = WMC_NewFullColormap(size, size);
  WMC_Color *c;
  if (!gfcm) return 0;
...
static int
read_logical_screen_descriptor(WMC_Stream *gfs, WMC_Reader *grr)
...
static int
read_compressed_image(WMC_Image *gfi, WMC_Reader *grr, int read_flags)
{
  if (grr->is_record) {
    ...
    if (read_flags & WMC_GIF_READ_CONST_RECORD) {
      gfi->compressed = (uint8_t*) &grr->v[image_pos];
      gfi->free_compressed = 0;
    } else {
      gfi->compressed = WMC_NewArray(uint8_t, gfi->compressed_len);
    ...
    uint32_t comp_len;
    uint8_t *comp = WMC_NewArray(uint8_t, comp_cap);
    int i;
    if (!comp) return 0;
	...
	comp_cap *= 2;
	WMC_ReArray(comp, uint8_t, comp_cap);
	if (!comp) return 0;
      }
      comp[comp_len] = i;
...
static int
uncompress_image(WMC_Context *gfc, WMC_Image *gfi, WMC_Reader *grr)
{
    if (!WMC_CreateUncompressedImage(gfi, gfi->interlace))
        return 0;
    gfc->width = gfi->width;
...
int
WMC_FullUncompressImage(WMC_Stream* gfs, WMC_Image* gfi,
                        WMC_ReadErrorHandler h)
{
  WMC_Context gfc;
  WMC_Reader grr;
  int ok = 0;
  ...
  gfc.stream = gfs;
  gfc.gfi = gfi;
  gfc.prefix = WMC_NewArray(WMC_Code, WMC_GIF_MAX_CODE);
  gfc.suffix = WMC_NewArray(uint8_t, WMC_GIF_MAX_CODE);
  gfc.length = WMC_NewArray(uint16_t, WMC_GIF_MAX_CODE);
  gfc.handler = h;
  ...
  WMC_DeleteArray(gfc.prefix);
  WMC_DeleteArray(gfc.suffix);
  WMC_DeleteArray(gfc.length);
...
static int
read_image(WMC_Reader *grr, WMC_Context *gfc, WMC_Image *gfi, int read_flags)
     /* returns 0 on memory error */
{
  uint8_t packed;
  ...
  if (read_flags & WMC_GIF_READ_COMPRESSED) {
    if (!read_compressed_image(gfi, grr, read_flags))
      return 0;
    if (read_flags & WMC_GIF_READ_UNCOMPRESSED) {
      WMC_Reader new_grr;
  ...
  } else if (read_flags & WMC_GIF_READ_UNCOMPRESSED) {
    if (!uncompress_image(gfc, gfi, grr))
      return 0;

  } else {
    /* skip over the image */
    uint8_t buffer[WMC_GIF_MAX_BLOCK];
...
static void
read_graphic_control_extension(WMC_Context *gfc, WMC_Image *gfi,
			       WMC_Reader *grr)
{
  uint8_t len;
  uint8_t crap[WMC_GIF_MAX_BLOCK];
...
static char *
suck_data(char *data, int *store_len, WMC_Reader *grr)
{
  uint8_t len = gifgetbyte(grr);
  int total_len = 0;

  while (len > 0) {
    WMC_ReArray(data, char, total_len + len + 1);
    if (!data) return 0;
...
static int
read_unknown_extension(WMC_Context* gfc, WMC_Reader* grr,
                       int kind, char* appname, int applength)
{
    uint8_t block_len = gifgetbyte(grr);
    uint8_t* data = 0;
    int data_len = 0;
    WMC_Extension *gfex = 0;

    while (block_len > 0) {
        WMC_ReArray(data, uint8_t, data_len + block_len + 2);
        if (!data)
            goto done;
    ...
    if (data)
        gfex = WMC_NewExtension(kind, appname, applength);
    if (gfex) {
        gfex->data = data;
        gfex->free_data = WMC_Free;
        gfex->length = data_len;
        gfex->packetized = 1;
        data[data_len] = 0;
        WMC_AddExtension(gfc->stream, gfc->gfi, gfex);
    }

 done:
    if (!gfex)
        WMC_DeleteArray(data);
    while (block_len > 0) {
        uint8_t buffer[WMC_GIF_MAX_BLOCK];
...
static int
read_application_extension(WMC_Context *gfc, WMC_Reader *grr)
{
  WMC_Stream *gfs = gfc->stream;
  uint8_t buffer[WMC_GIF_MAX_BLOCK + 1];
  uint8_t len = gifgetbyte(grr);
...
static int
read_comment_extension(WMC_Image *gfi, WMC_Reader *grr)
{
  int len;
  WMC_Comment *gfcom = gfi->comment;
  char *m = suck_data(0, &len, grr);
  if (m) {
    if (!gfcom)
      gfcom = gfi->comment = WMC_NewComment();
    if (!gfcom || !WMC_AddCommentTake(gfcom, m, len))
      return 0;
  }
  return 1;
}
...
static WMC_Stream *
read_gif(WMC_Reader *grr, int read_flags,
	 const char* landmark, WMC_ReadErrorHandler handler)
{
  WMC_Stream *gfs;
  WMC_Image *gfi;
  WMC_Context gfc;
  ...
  while (!gifeof(grr)) {
    ...
    switch (block) {
     ...
     case ',': /* image block */
      ...
      last_name = 0;
      if (!WMC_AddImage(gfs, gfi))
          goto done;
      else if (!read_image(grr, &gfc, gfi, read_flags)) {
          WMC_RemoveImage(gfs, gfs->nimages - 1);
          goto done;
      }

      gfc.gfi = gfi = WMC_NewImage();
      if (!gfi)
          goto done;
      break;
  ...
  if (gfs && gfc.errors[1] == 0
      && !(read_flags & WMC_GIF_READ_TRAILING_GARBAGE_OK)
      && !grr->eofer(grr))
...
WMC_Stream *
WMC_FullReadFile(FILE *f, int read_flags,
		 const char* landmark, WMC_ReadErrorHandler h)
{
  WMC_Reader grr;
  if (!f) return 0;
  grr.f = f;
...
WMC_Stream *
WMC_FullReadRecord(const WMC_Record *gifrec, int read_flags,
		   const char* landmark, WMC_ReadErrorHandler h)
{
  WMC_Reader grr;
  if (!gifrec) return 0;
  ...
  if (read_flags & WMC_GIF_READ_CONST_RECORD)
    read_flags |= WMC_GIF_READ_COMPRESSED;
  return read_gif(&grr, read_flags, landmark, h);
}


#undef WMC_ReadFile
#undef WMC_ReadRecord

WMC_Stream *
WMC_ReadFile(FILE *f)
{
  return WMC_FullReadFile(f, WMC_GIF_READ_UNCOMPRESSED, 0, 0);
}

WMC_Stream *
WMC_ReadRecord(const WMC_Record *gifrec)
{
  return WMC_FullReadRecord(gifrec, WMC_GIF_READ_UNCOMPRESSED, 0, 0);
}

void
WMC_SetErrorHandler(WMC_ReadErrorHandler handler) {
